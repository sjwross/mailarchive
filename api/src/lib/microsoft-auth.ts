...
export async function getAuthUrl(state: string): Promise<string> {
  ...
  const authUrl = await getPca().getAuthCodeUrl(authCodeUrlParameters);
...
export async function acquireTokenByCode(code: string): Promise<{
  ...
  const result = await getPca().acquireTokenByCode(tokenRequest);

  // Extract refresh token from token cache
  let refreshToken: string | null = null;
  try {
    const tokenCache = getPca().getTokenCache().serialize();
...
export async function refreshAccessToken(refreshToken: string): Promise<{
  ...
  try {
    ...
    const result = await getPca().acquireTokenByRefreshToken(tokenRequest);
